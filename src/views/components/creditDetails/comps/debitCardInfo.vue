...
export default {
  ...
  computed: {
    ...
    modify () {
      let modify = true
      try {
        const loanInfo = (this.$store.state.dataType || {}).loanCardList || {}
  ...
  created () {
    window.storeGlobalIoc.allDebitCardClear = this.allLoanClear.bind(this)
  },
  methods: {
    // 判断是否未激活
    ...
    doAddLoanCardList (idx) {
      const list = this.loanCardListInfo || []
      list.splice(idx + 1, 0, {
        id: new Date().getTime() + 1000
      })
      this.$store.commit('setLoanCardInfo', list)
    },
    // 删除当前的贷记卡列表
    doDeleteLoanCardList (idx) {
      const layer = window.layer
      const layIndex = layer.confirm('确定删除当前贷记卡列表？', () => {
        const list = this.loanCardListInfo || []
        list.splice(idx, 1)
        this.$store.commit('setLoanCardInfo', list)
    ...
    doAddLoanOverdue (index, idx) {
      const list = this.loanCardListInfo[index].latest5yearOverdueDetailList || []
      list.splice(idx + 1, 0, {
        id: new Date().getTime() + 1000
      })
      this.$store.commit('setLoanCardOverdueInfo', {
        idx: index,
        list
      })
    },
    ...
    doSave (bool) {
      let valid = true
      const loanCardList = this.loanCardListInfo || []
      loanCardList.forEach((loanData, index) => {
        ...
        if (!this.validateInput(loanData, config, bool, `input-loan-${loanData.id}-`, `贷记卡${index + 1}`)) {
          valid = false
        }
        // 验证逾期部分
        // 验证逾期部分
        ...
        if (!this.validateInputList(overdueList, overdueConfig, bool, `li-overdue-input-${loanData.id}-`, `贷记卡${index + 1}-逾期`)) {
          valid = false
        }
        ...
        if (!this.validateInputList(overdueList1, overdueConfig1, bool, `li-overdue-input-${loanData.id}-`, `贷记卡${index + 1}-逾期`)) {
          valid = false
        }
      })
      return valid
    ...
    allLoanClear (start = 1, end = 999999, name = '销户') {
      const list = (this.loanCardListInfo || []).map((item, idx) => {
