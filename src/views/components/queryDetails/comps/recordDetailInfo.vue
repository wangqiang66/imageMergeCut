...
<template>
  <div class="record-detail-info-box">
    <div class="row list-title-wrap">
      <h5 class="title flex1">机构查询记录明细</h5>
    ...
    <div class="table-wrap div-wrap">
      ...
      <ul
        v-for="(item, idx) in recordDetailInfo"
...
export default {
  ...
  created () {
    window.storeGlobalIoc.doReplaceRecordDetailList = this.doReplaceList.bind(this)
  },
  methods: {
    doDeleteAllList () {
      ...
      layer.prompt({
        ...
        layer.confirm(`确定删除${tip}机构查询记录明细？`, (index1) => {
          this.$store.commit('setRecordDetailInfo', list)
          layer.close(index1)
          layer.close(index)
        })
      })
    ...
    // 添加逾期信息列表
    doAddList (idx) {
      const list = this.recordDetailInfo || []
      list.splice(idx + 1, 0, {
    ...
    // 保存验证
    doSave (bool) {
      return this.validateInputList(this.recordDetailInfo, this.config, bool, 'input-', '机构查询记录明细')
    },
    // 替换多条记录
