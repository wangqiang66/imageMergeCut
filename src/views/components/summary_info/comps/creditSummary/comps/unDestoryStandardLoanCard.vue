<!--
  function: 未销户准贷记卡信息汇总
  author  : wq
  update  : 2018/8/15 11:22
-->
<template>
  <div>
    <h6 class="title">未销户准贷记卡信息汇总</h6>
      ...
      <template
        ...
        <ul
          ...
          <li
            ...
            :class="[itm.width ? 'flex' + itm.width : 'flex1']">
            <form-input
              v-model="unDestoryStandardLoanCardInfo[itm.key]"
              :type="itm.type"
              :list="itm.list"
...
export default {
  name: 'un-destory-standard-loan-card',
  mixins: [InputMixin],
  data () {
    return {
  ...
  computed: {
    ...mapGetters([
      'unDestoryStandardLoanCardInfo'
    ])
  },
  methods: {
    doSave (bool) {
      return this.validateInput(this.unDestoryStandardLoanCardInfo, this.config, bool, 'input-', '未销户准贷记卡信息汇总')
    }
  }
}
</script>
